...
class DefectSeverityStatusBarChart {
	Object[] defects
	Object[] cache
	DefaultCategoryDataset dataset

	public DefectSeverityStatusBarChart(Object[] defects){
		this.defects = defects
		dataset = new DefaultCategoryDataset()
		def mapsv = [], mapst = []
		defects.each{ obj ->
			Bug defect = (Bug)obj
			if (!mapsv.contains(defect.getSeverity()))
				mapsv.add defect.getSeverity()
			if (!mapst.contains(defect.getStatus()))
				mapst.add defect.getStatus()
		}

		mapsv.each{sv ->
			cache = defects.findAll{ d -> d.severity.equals(sv) }
			mapst.each{st ->
	...
	void createGraph(){
		...
		def chart = ChartFactory.createStackedBarChart3D(*labels, dataset, Orientation.HORIZONTAL, *options)
