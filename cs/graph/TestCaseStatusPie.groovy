...
class TestStatusChart {
	...
	public TestStatusChart(Object[] tests){
		this.tests = tests
		dataset = new DefaultPieDataset()
		def map = []
		tests.each{ obj ->
			Test test = (Test)obj
			if (!map.contains(test.getStatus()))
				map.add test.getStatus()
		}

		map.each{it ->
			dataset.setValue(it.toString(), tests.findAll{ d -> d.getStatus().equals(it) }.size())
		}
		createGraph()
	}
