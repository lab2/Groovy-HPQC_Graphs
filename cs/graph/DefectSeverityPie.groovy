...
class DefectSeverityChart {
	Object[] defects
	DefaultPieDataset dataset

	public DefectSeverityChart(Object[] defects){
		...
		defects.each{ obj ->
			Bug defect = (Bug)obj
			if (!map.contains(defect.getSeverity()))
				map.add defect.getSeverity()
		}

		map.each{it ->
			dataset.setValue(it.toString(), defects.findAll{d -> d.getSeverity().equals(it)}.size())
		}
		createGraph()
	}
