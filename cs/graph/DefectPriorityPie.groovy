...
class DefectPriorityChart {
	Object[] defects
	DefaultPieDataset dataset

	public DefectPriorityChart(Object[] defects){
		...
		defects.each{ obj ->
			Bug defect = (Bug)obj
			if (!map.contains(defect.getPriority()))
				map.add defect.getPriority()
		}

		map.each{it ->
			dataset.setValue(it.toString(), defects.findAll{ d -> d.getPriority().equals(it) }.size())
		}
		createGraph()
	}
